...
bool CharToCString(char * pchText, CString & strText, UINT nCodePage)
{
	if(nullptr == pchText) {
		RETURN_FALSE;
	}
...
// 한국어를 사용하는 윈도우에서는 멀티바이트일때 CP949를 디폴트로 사용한다. 
// 따라서 WideCharToMultiByte 사용시 CodePage 인자로 UTF-8을 넣는게 아니라 CP_ACP를 넣으면 CP949로 인코딩된다.
bool CStringToChar(CString strText, char *& pchText, int & nTextSize, UINT nCodePage)
{
...
MbCartPoint3D GetMbCartPoint3D(A3DVector3dData cVector, double dScale)
{
...
{
	MbVector3D cXAxis = GetMbVector3D(cTransformationData.m_sXVector);
	MbVector3D cYAxis = GetMbVector3D(cTransformationData.m_sYVector);
	//MbVector3D cScale = Rt3dxUtility::GetMbVector3D(cTransformationData.m_sScale);
	MbCartPoint3D cOrigin = GetMbCartPoint3D(cTransformationData.m_sOrigin, dParentScale);
