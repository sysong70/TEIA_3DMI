...
bool H3DF::WindowKeyImpl::IsInitNavigationCube()
{
	return m_pcNaviCube->IsInitialized();
}

NavigationCube & H3DF::WindowKeyImpl::GetNavigationCube()
{
	return *m_pcNaviCube;
}

void H3DF::WindowKeyImpl::InitNavigationCube(int nWidth, int nHeight)
{
	m_pcNaviCube->SetView(m_pcBaseView);
...
void H3DF::WindowKeyImpl::Resize(int x, int y)
{
	GetBaseView()->SetXYSizeOverride(x, y);

	if (x > 0 && y > 0 && true == IsInitNavigationCube()) {
		m_pcNaviCube->OnSize(x, y);
	}
}

void H3DF::WindowKeyImpl::ViewReady()
{
...
// #Window: 1. BaseView를 초기화 하는 부분
bool H3DF::WindowKeyImpl::Init(H3DF::Model * pcInModel, const char * pchInDriverType, CStringA strInInstanceName, H3DF::WindowHandle nInWindowHandle)
{
...
// 1.1 Gpu 설정
void H3DF::WindowKeyImpl::SetGpu(CString strGpu)
{
	char gpu_to_use[256];
...
void H3DF::WindowKeyImpl::SetDriverOption()
{
...
void H3DF::WindowKeyImpl::SetAntiAliasOption()
{
	char chDriverOpts[MVO_BUFFER_SIZE] = { 0 };
...
// 1.3 투명도 적용 방법 설정
void H3DF::WindowKeyImpl::SetTransparency()
{
	char chText[4096];
	char chSorting[4096];
...
void H3DF::WindowKeyImpl::SetupFrameRateMode()
{
...
void H3DF::WindowKeyImpl::SetWindowBackGroundColor(COLORREF nNewTopColor, COLORREF nNewBottomColor, bool bEmitMessage)
{
...
void H3DF::WindowKeyImpl::SetSceneFont(CString csFontName, CString csFontSize, CString csFontUnits)
{
...
void H3DF::WindowKeyImpl::SetMarkupColor(COLORREF new_color, bool emit_message)
{
...
void H3DF::WindowKeyImpl::SetShadowColor(RGBAColor cInColor)
{
...
void H3DF::WindowKeyImpl::SetViewAxis()
{
	DEBUG_VALID(m_pcBaseView);
	/*
		char text[4096];
		HVector front, top;
...
void H3DF::WindowKeyImpl::event_checker(HIC_Rendition const * nr)
{
	//MSG msg;
...
void H3DF::WindowKeyImpl::SetSelectionControl(WindowKey const & cInWindow)
{
	...
	H3DF::BaseView * pcBaseView = pcWindowImpl->GetBaseView();
	...
	pcWindowImpl->m_pcSelectionSet = pcBaseView->GetSelection();

	pcWindowImpl->m_pcSelection = new SelectionControl(cInWindow);
	SelectionControlImpl * pcSelectionImpl = dynamic_cast<SelectionControlImpl *>(pcWindowImpl->m_pcSelection->GetImpl());
	pcSelectionImpl->m_pcSelectionSet = pcBaseView->GetSelection();

	pcWindowImpl->m_pcSelectionOptions = new SelectionOptionsControl(cInWindow);
	SelectionOptionsControlImpl * pcSelectionOptionsImpl = dynamic_cast<SelectionOptionsControlImpl *>(pcWindowImpl->m_pcSelectionOptions->GetImpl());
	pcSelectionOptionsImpl->m_pcSelectionSet = pcBaseView->GetSelection();
}
...
void H3DF::WindowKeyImpl::SetHighlightControl(WindowKey const & cInWindow)
{
	...
	pcWindowImpl->m_pcHighlight = new HighlightControl(cInWindow);
}

//== Operator 관련 함수 ==============================================================================

void H3DF::WindowKeyImpl::SetDefaultOperator()
{
...
void H3DF::WindowKeyImpl::LocalSetOperator(HBaseOperator * pcNewOperator)
{
...
H3DF::BaseView * H3DF::WindowKeyImpl::GetBaseView() 
{ 
	DEBUG_VALID(m_pcBaseView);
	return (H3DF::BaseView *) m_pcBaseView; 
}
