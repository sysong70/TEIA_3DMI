...
#if _MSC_VER
static int big_fseek(FILE* file, int64_t offset, int origin) { return _fseeki64(file, offset, origin); }

static int64_t big_ftell(FILE* file) { return _ftelli64(file); }
#else
static int big_fseek(FILE* file, int64_t offset, int origin) { return fseeko(file, offset, origin); }

static int64_t big_ftell(FILE* file) { return ftello(file); }
#endif
...
class PCPoint {
public:
    double bbox[6]; // bbox for bsp is now doubles, can't use 2 copies of the point itself
    HPoint pt;
    ...
    PCPoint(
        float x = 0.0f, float y = 0.0f, float z = 0.0f, int i = 0, unsigned char r = 0, unsigned char g = 0, unsigned char b = 0)
    {
        pt = HPoint(x, y, z);
        bbox[0] = bbox[3] = pt.x;
        bbox[1] = bbox[4] = pt.y;
...
enum ReadWriteStatus { OK, ERROR1, END_OF_FILE };
...
public:
    BufferedFile(FILE * fp_in) { m_fp = fp_in; }
...
public:
    BufferedWriter(FILE * fp_in) : BufferedFile(fp_in) { m_write_pos = 0; }
...
public:
    ...
    BSPData(PointCloud * this_io_in, HPointCloudOptions * m_point_cloud_options_in = 0)
    {
...
char const* PointCloud::GetInputTypesString() { return "pts,ptx,xyz"; }

void PointCloud::RegisterInputHandlerTypes()
{
...
bool PointCloud::Initialize(wchar_t const* filename)
{
    ...
    while (true) {
        ...
        while (read_pos < amount_read) {
            ...
            size_t line_len = 0;
            {
                ...
                if (new_line_pos) {
                    ...
                    if (no_more_reads) {
                        ...
                        size_t lamount_unprocessed = amount_read - read_pos;
                        big_fseek(m_file, -static_cast<int64_t>(lamount_unprocessed), SEEK_CUR);
                        break;
                    }
                }
            }
            ...
            int b = 0;
            int result;
            //HCLOCALE(result = sscanf(read_buffer + read_pos, "%f%*c%f%*c%f%*c%f%*c%d%*c%d%*c%d", &x, &y, &z, &intensity, &r, &g, &b););
            ...
            if (fabs(x) < epsilon && fabs(y) < epsilon && fabs(z) < epsilon && fabs(intensity - 0.5f) < epsilon && r == 0.0f &&
                g == 0.0f && b == 0.0f) {
                ...
                if (result == 3) {
                    ...
                    cpt.bit_field &= ~VALID_INTENSITY;
                    // since there is no intensity, the RGB fields are shifted
                    cpt.rgb_color.blue = cpt.rgb_color.green;
                    cpt.rgb_color.green = cpt.rgb_color.red;
 ...
 //               LogManager::Log(LOGMANAGER_3DF_LOG_ID, L"Percent Progress: %d%%", (int)(progress * 100));
                prev_pos = curr_pos;
            }
...
float PointCloud::GetScaledIntensity(PCPoint const* cpt)
{
        ...
        while (true) {
            ...
            if (cpt.bit_field & VALID_POINT) {
                ...
                if (cpt.bit_field & VALID_RGB) {
                    ...
                    // PCPoint *new_cpt = point_pool->GetNewPoint(cpt);
                    gray_point_bsp->Insert(new_cpt, (double*)new_cpt);
                }
                else {
            ...
            if (point_count == POINT_LIST_CHUNK || eof) {
                ...
                if (m_point_cloud_options->m_numBuckets == 0) {
                    ...
                    if (m_point_cloud_file_type == PTX)
                        ...
                        m_point_cloud_options->m_numBuckets =
                            m_point_cloud_options->m_highIntensityValue - m_point_cloud_options->m_lowIntensityValue;
...
void PointCloud::WriteShellsToFile(VBSP<PCPoint*>* bsp, int bucket, void const* const user_data)
{
    ...
    if (bucket >= 0) {
        ...
        if (bsp_data->m_bucket_seg_hash->LookupItem(bucket, &bucket_seg_key) == VHASH_STATUS_FAILED) {
            ...
            HC_Set_Color(
                H_FORMAT_TEXT("r = %g g = %g b = %g", bsp_data->m_gray_scale, bsp_data->m_gray_scale, bsp_data->m_gray_scale));
            HC_Close_Segment();
...
VBSPStatus PointCloud::GetShellsFromBSP(PCPointContainer* container, void* user_data)
{
    BSPData* bsp_data = (BSPData*)user_data;
...
bool PointCloud::InsertShells(BSPData* bsp_data)
{
...
void PointCloud::DeleteBSPs(VBSP<PCPoint*>* bsp, int bucket, void const* const user_data)
{
    UNREFERENCED(bucket);
    UNREFERENCED(user_data);
