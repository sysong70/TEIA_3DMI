...
public:
	...
	static OdEdPointTrackerPtr create(const OdGePoint3d& basePoint, int options, OdEdRealTracker* pTracker, OdRxObject* pRxDb)
	{
...
class DbPointToOrientTracker : public OdEdPointTracker
{
	OdGePoint3d m_basePoint;
	OdEdRealTracker* m_pClientTracker;
	OdDbDatabase* m_pDb;
...
public:
	...
	static OdEdPointTrackerPtr create(const OdGePoint3d& basePoint, OdEdRealTracker* pClientTracker, OdDbDatabase* pDb)
	{
		OdSmartPtr<DbPointToOrientTracker> pRes = OdRxObjectImpl<DbPointToOrientTracker>::createObject();
		pRes->m_basePoint = basePoint;
		pRes->m_pClientTracker = pClientTracker;
		pRes->m_pDb = pDb;

		return pRes;
	}

	double getAngle(const OdGePoint3d& pt) const
	{
		return getAngle(pt, m_basePoint, m_pDb);
	}

	static double getAngle(const OdGePoint3d& pt, const OdGePoint3d& basePt, OdDbDatabase* pDb)
	{
		OdGeMatrix3d xToUCS;
		if (pDb->getTILEMODE()) {
			xToUCS.setCoordSystem(pDb->getUCSORG(), pDb->getUCSXDIR(), pDb->getUCSYDIR(),
				pDb->getUCSXDIR().crossProduct(pDb->getUCSYDIR()));
		}
		else {
			xToUCS.setCoordSystem(pDb->getPUCSORG(), pDb->getPUCSXDIR(), pDb->getPUCSYDIR(),
				pDb->getPUCSXDIR().crossProduct(pDb->getPUCSYDIR()));
		}
		xToUCS.invert();

		OdGePoint3d ptUCS = xToUCS * pt; ptUCS.z = 0.;
		OdGePoint3d ptBaseUCS = xToUCS * basePt; ptBaseUCS.z = 0.;

		return OdGeVector3d::kXAxis.angleTo(ptUCS - ptBaseUCS, OdGeVector3d::kZAxis) - pDb->getANGBASE();
	}

	void setValue(const OdGePoint3d& pt)
	{
		m_pClientTracker->setValue(getAngle(pt));
	}

	int addDrawables(OdGsView* pView)
	{
		return m_pClientTracker->addDrawables(pView);
	}

	void removeDrawables(OdGsView* pView)
	{
		m_pClientTracker->removeDrawables(pView);
	}
};

//--------------------------------------------------------------------------------------------------

class ComboTracker : public OdEdPointTracker
{
public:

	OdSmartPtr<OdEdPointTracker> pT1, pT2;
	...
	{
		return pT1->addDrawables(pView) + pT2->addDrawables(pView);
	}

	virtual void removeDrawables(OdGsView* pView)
	{
		pT1->removeDrawables(pView);
		pT2->removeDrawables(pView);
	}
};
...
class OdEdTrackerProxy : public OdEdStringTracker
{
public:
	...
	virtual int addDrawables(OdGsView* pView)
	{
		return tracker->addDrawables(pView);
	}

	virtual void removeDrawables(OdGsView* pView)
	{
		tracker->removeDrawables(pView);
	}

	virtual void setValue(const OdString&) { }
...
protected:
	...
	virtual void subViewportDraw(OdGiViewportDraw* pVd) const ODRX_OVERRIDE
	{
		if (m_bApplicable) {
			...
			OdGiDrawFlagsHelper dfh(pVd->subEntityTraits(), OdGiSubEntityTraits::kDrawSolidFill | OdGiSubEntityTraits::kDrawPolygonFill);
...
public:
	...
	int addDrawables(OdGsView* pView)
	{
		m_bApplicable = (
			pView != nullptr &&
			pView->device() != nullptr &&
			pView->device()->properties().isNull() == false &&
...
protected:
	...
	{
		if (pEntity.isNull() == false && pEntity->visibility() == OdDb::kVisible) {
			OdDbLayerTableRecordPtr pLayer = OdDbLayerTableRecord::cast(pEntity->layerId().openObject(OdDb::kForRead));
			if (pLayer->isFrozen() == false && pLayer->isOff() == false) {
				return true;
			}
		}
...
public:
	...
	static void selectLast(OdDbDatabase* pDb, OdDbObjectIdArray& ids)
	{
		...
		while (pIt->done() == false) {
			...
			if (pDb != nullptr && (cntr % DBCC_PAGE_EACH_OBJECT)) {
				pDb->pageObjects();
				cntr = 0;
			}
	...
	static void selectPolyline(OdSmartPtr<OdEdBaseIO> pIoStream, OdDbUserIO* pIO, OdDbVisualSelection::SubentSelectionMode& ssm, OdGePoint3dArray& pts)
	{
		...
		while (true) {
			...
			try {
				pts.append(pIO->getPoint(OD_T("Specify next fence point:"), OdEd::kGptNoLimCheck | OdEd::kGptNoUCS | OdEd::kInpThrowEmpty, 0, OD_T("Undo"), pTracker));
	...
	static void selectPolygon(OdSmartPtr<OdEdBaseIO> pIoStream, OdDbUserIO* pIO, OdDbVisualSelection::SubentSelectionMode& ssm, OdGePoint3dArray& pts)
	{
		...
		while (true) {
			...
			try {
				OdGePoint3d newPoint = pIO->getPoint(OD_T("Specify endpoint of line:"), OdEd::kGptNoLimCheck | OdEd::kGptNoUCS | OdEd::kInpThrowEmpty, 0, OD_T("Undo"), pTracker);
...
OdString next(OdString& list, const OdChar* delim = (const OdChar*)L",")
{
...
void printInvalidInputMessage(OdDbUserIO& io, const OdString& keywords)
{
	io.putError(keywords.isEmpty() ? OD_T("Invalid point.") : OD_T("Point or option keyword required."));
}
...
static bool disallowedOptionZeroNeg(OdEdBaseIO* pIoStream, int options, double& val)
{
	if (GETBIT(options, OdEd::kGrlNoZero) && OdZero(val)) {
		pIoStream->putError(OD_T("Value must be nonzero."));
		return true;
	}

	if (GETBIT(options, OdEd::kGrlNoNeg) && val < 0.) {
		pIoStream->putError(OD_T("Value must be not negative."));
		return true;
	}
...
OdString findDefOptionPrompt(const OdString& sPrompt, int defVal)
{
	int s = sPrompt.find(L'[');
	if (s > 0) {
		for (int v = 0, i = s + 1, j; ; i = j + 1, ++v) {
			j = sPrompt.find(L'/', i);
...
OdEdCommandContextPtr ExDbCommandContext::cloneObject(OdEdBaseIO* pIOStream, OdRxObject* pRxDb)
{
	OdDbCommandContextPtr pRes = OdRxObjectImpl<ExDbCommandContext, OdDbCommandContext>::createObject();
	ExDbCommandContext* pDbCmdCtx = static_cast<ExDbCommandContext*>(pRes.get());
...
OdSelectionSetPtr ExDbCommandContext::box(const OdGePoint3d& pt1, OdDbVisualSelection::SubentSelectionMode ssm)
{
	...
	public:
		...
		~EnableEnhRectFrame()
		 {
			m_pCmdCtx->setArbitraryData(OD_T("ExDbCommandContext_EnhRectFrame"), nullptr);
		}
	}
	_enhRect(this);

	OdGePoint3d pts[2] = { pt1, dbUserIO()->getPoint(OD_T("Specify opposite corner:"), OdEd::kGptNoLimCheck | OdEd::kGptNoUCS | OdEd::kGptRectFrame) };
...
void ExDbCommandContext::highlight(OdSelectionSet* pSSet, bool bDoIt) const
{
	...
	for (; !pIter->done(); pIter->next()) {
		...
		if ((cntr % DBCC_PAGE_EACH_OBJECT) && (pDb = OdDbObjectId(pIter->id()).database())) {
			pDb->pageObjects();
			cntr = 0;
		}
...
void ExDbCommandContext::highlight(OdSelectionSetIterator* pIter, bool bDoIt) const
{
	OdDbObjectPtr pObj = OdDbObjectId(pIter->id()).safeOpenObject();
	if (pObj->isKindOf(OdDbEntity::desc()) == false) {
		return;
	}
...
void ExDbCommandContext::highlight(const OdDbBaseFullSubentPath& subEntPath, bool bDoIt) const
{
	OdDbEntityPtr pEnt = OdDbEntity::cast(OdDbObjectId(subEntPath.objectIds().first()).openObject());
	if (pEnt.get() != nullptr) {
		OdDbFullSubentPath dbSubEntPath(subEntPath);
		pEnt->highlight(bDoIt, &dbSubEntPath);
	}
}
...
bool ExDbCommandContext::merge(OdSelectionSet* pRes, int options, const OdSelectionSet* pSSet, OdSSetTracker* pTracker, OdEdUserIO* pIO)
{
	...
	const OdRxObject* pRxDb = pRes->baseDatabase();
	...
	OdUInt32 numResSubentities = pRes->subentCount();
	OdUInt32 numSSetEntities = pSSet->numEntities();
	OdUInt32 limitGripObj = getGripObjLimit(pRxDb);
	...
	if (m_pIoStream->getKeyState() & OdEdBaseIO::kShiftIsDown) {
		bAppending = !bAppending;
	}

	OdDbObjectId BTRId;
	OdDbDatabase* pDb = OdDbDatabase::cast(pRxDb).get();
	if (pDb != nullptr && GETBIT(options, OdEd::kSelAllowInactSpaces) == false) {
	...
	for (pIter = pSSet->newIterator(); !pIter->done(); pIter->next()) {
		...
		ODA_ASSERT_ONCE(id && pRxDb == baseDatabaseBy(id));
		...
		if (bMember && GETBIT(options, OdEd::kSelAllowSubents)) {
			for (OdUInt32 se = 0; se < pIter->subentCount(); se++) {
				...
				if (pRes->isMember(subEntPath) == false) {
					bMember = false;
					break;
				}
			}
		...
		if (pDb != nullptr) {
			...
			if (pEnt.get() != nullptr) {
				...
				OdDbLayerTableRecordPtr ltr = pEnt->layerId().openObject();
				if (GETBIT(options, OdEd::kSelAllowLocked) == false && ltr.isNull() == false && ltr->isLocked()) {
					++nOnLockedLayer;
					continue;
				}
				else if (GETBIT(options, OdEd::kSelAllowInactSpaces) == false && notInCurrentSpace) {
					++nNotInCurrentSpace;
					continue;
				}
				else if (bPsVp == false && GETBIT(options, OdEd::kSelAllowPSVP) == false && ::isOverallVp(pEnt)) {
					bPsVp = true;
					continue;
				}
			}
			else if (GETBIT(options, OdEd::kSelAllowObjects) == false) {
				++nObjects;
				continue;
			}
		}

		bool bTrackSubentities = pTracker && pTracker->trackSubentities();
		OdUInt32 numSubent = pIter->subentCount();

		if (bAppending) {
			if (pTracker != nullptr || bTrackSubentities && numSubent > 0 || pTracker->append(id, pIter->method())) {
				bool bCanHighlight = (numResEntities < limitGripObj && numSSetEntities < limitGripObj);
				if (bCanHighlight && (!bTrackSubentities || numSubent == 0)) {
					highlight(pIter, bAppending);
				}

				pRes->append(id, pIter->method());
				for (OdUInt32 se = 0; se < numSubent; se++) {
					OdDbBaseFullSubentPath subEntPath;
					if (pIter->getSubentity(se, subEntPath)) {
						if (pTracker != nullptr || bTrackSubentities == false || pTracker->append(subEntPath, pIter->method())) {
							if (bTrackSubentities) {
								highlight(subEntPath, bAppending);
							}
			...
			if (pTracker != nullptr || bTrackSubentities && numSubent > 0 || pTracker->remove(id, pIter->method())) {
				if (bTrackSubentities == false || numSubent == 0) {
					highlight(pIter, bAppending);
				}

				if (GETBIT(options, OdEd::kSelAllowSubents) || GETBIT(options, OdEd::kSelAllowSubentsAlways)) {
					for (OdSelectionSetIteratorPtr pResIter = pRes->newIterator(); !pResIter->done(); pResIter->next()) {
						if (pResIter->id() != id) {
							continue;
						}
						...
						if (pResIter->subentCount()) {
							for (OdUInt32 se = 0; se < numSubent; se++) {
								OdDbBaseFullSubentPath subEntPath;
								if (pIter->getSubentity(se, subEntPath)) {
									if (pTracker != nullptr || bTrackSubentities == false || pTracker->remove(subEntPath, pIter->method())) {
										if (bTrackSubentities) {
											highlight(subEntPath, bAppending);
										}
	...
	return (numResEntities != pRes->numEntities() || numResSubentities != pRes->subentCount());
}



OdSelectionSetPtr ExDbCommandContext::select(const OdString& sPrompt, int options, const OdSelectionSet* pDefVal, const OdString& sKeywords, OdSSetTracker* pBaseTracker, OdGePoint3dArray* ptsPointer)
{
		...
		try {
			try {
				if (GETBIT(options, OdEd::kSelPickLastPoint)) {
					...
					pts[0] = pIO->getPoint(prompt, OdEd::kGptNoUCS | OdEd::kGptNoLimCheck | OdEd::kInpThrowEmpty | OdEd::kInpThrowOther, 0, keywords);
				}
				...
				bool bSelected = merge(pRes, options, pSSet, pTracker, pIO);

				if (bSelected == false && GETBIT(options, OdEd::kSelSingleEntity) == false) {
					...
					if (ssm == OdDbVisualSelection::kEnableSubents && GETBIT(options, OdEd::kSelAllowSubentsAlways)) {
						pSSet = OdDbSelectionSet::select(database()->activeViewportId(), 1, &pts[0], OdDbVisualSelection::kPoint, OdDbVisualSelection::kDisableSubents);
						bSelected = merge(pRes, options, pSSet, pTracker, pIO);
					}
					if (bSelected == false) {
						pSSet = box(pts[0], ssm);
						bSelected = merge(pRes, options, pSSet, pTracker, pIO);
	...
	if (GETBIT(options, OdEd::kSelLeaveHighlighted) == false && pRes->numEntities() < getGripObjLimit(pRes->baseDatabase())) {
		highlight(pRes, false);
	}
...
OdGePoint3d ExDbCommandContext::getPointOrReal(const OdString& prompt, int options, const OdString& keywords, OdResBuf::ValueType realType, OdEdPointTracker* pPtTracker)
{
	while (true) {
		try {
			return getPoint(prompt, options | OdEd::kInpThrowOther, 0, keywords, pPtTracker);
		}
			...
			if (disallowedOptionZeroNeg(m_pIoStream, options, real.val)) {
				continue;
			}
...
double ExDbCommandContext::getReal(const OdString& sPrompt, int options, double defVal, OdResBuf::ValueType vt, const OdString& sKeywords, OdEdRealTracker* pTracker)
{
	OdString prompt(sPrompt);
	if (prompt.isEmpty()) {
	...
	if (GETBIT(options, OdEd::kGdsFromLastPoint) == false) {
		try {
			basePt = getPointOrReal(prompt, options | OdEd::kInpThrowEmpty, sKeywords, vt, 0);
		}
...
double ExDbCommandContext::getDist(const OdString& sPrompt, int options, double defVal, const OdString& sKeywords, OdEdRealTracker* pTracker)
{
	OdString prompt(sPrompt);
	if (prompt.isEmpty()) {
		prompt = OD_T("Enter a distance:");
...
double ExDbCommandContext::getAngle(const OdString& sPrompt, int options, double defVal, const OdString& sKeywords, OdEdRealTracker* pTracker)
{
	OdString prompt(sPrompt);
	...
	double dAngleBase = 0.0;
	double dAngle = defVal;
	OdGePoint3d basePt = dbUserIO()->getLASTPOINT();

	OdDbDatabasePtr pDb = OdDbDatabase::cast(m_pDb);
	if (pDb.get()) {
		dAngleBase = database()->getANGBASE();
	}

	OdEdPointTrackerPtr pPtTracker;
	if (pTracker != nullptr) {
		pPtTracker = DbPointToOrientTracker::create(basePt, pTracker, pDb);
	}

	while (true) {
		try {
			OdGePoint3d pt = getPoint(prompt, options | OdEd::kInpThrowEmpty, 0, sKeywords, pPtTracker);

			return DbPointToOrientTracker::getAngle(pt, basePt, pDb);
		}
		catch (const OdEdOtherInput& other) {
			OdString sInput = other.string();

			try {
				dAngle = baseFormatter().unformatAngle(sInput);
				dAngle -= dAngleBase;
			}
			catch (const OdError& e) {
				if (e.code() != eInvalidInput) {
				...
				putError(sKeywords.find(L' ') < 0 ? L"Requires valid numeric angle." : L"Requires valid numeric angle or optional keyword.");
			}
		}
		catch (const OdEdEmptyInput&) {
			if (GETBIT(options, OdEd::kInpThrowEmpty)) {
...
OdEdPointDefTrackerPtr ExDbCommandContext::createRubberBand(const OdGePoint3d& base, OdGsModel* pModel) const
{
...
OdEdPointDefTrackerPtr ExDbCommandContext::createRectFrame(const OdGePoint3d& base, OdGsModel* pModel) const
{
	// Create enhanced rect frame if requested
	{
		OdRxObjectPtr pArbData = arbitraryData(OD_T("ExDbCommandContext_EnhRectFrame"));
		if (pArbData.isNull() == false) {
...
OdGePoint3d ExDbCommandContext::getPoint(const OdString& sPrompt, int options, const OdGePoint3d* pDefVal, const OdString& keywords, OdEdPointTracker* pTracker)
{
	OdString prompt(sPrompt);
	...
	if (GETBIT(options, OdEd::kGptRectFrame)) {
		pPredef = createRectFrame(dbUserIO()->getLASTPOINT());
	}
	else if (GETBIT(options, OdEd::kGptRubberBand)) {
		pPredef = createRubberBand(dbUserIO()->getLASTPOINT());
	}

	if (pPredef.get()) {
		if (pTracker != nullptr) {
			OdSmartPtr<ComboTracker> pCombo = OdRxObjectImpl<ComboTracker>::createObject();
			pCombo->pT1 = pPredef;
			pCombo->pT2 = pTracker;
			pPredef = pCombo;
		}
	...
	while (true) {
		...
		try {
			...
			if (GETBIT(options, OdEd::kInpThrowEmpty)) {
				...
				::printInvalidInputMessage(*dbUserIO(), keywords);
				continue;
			}
		}
		catch (const OdEdOtherInput& other) {
			if (!other.string().isEmpty()) {
				...
				try {
					...
					OdString sInput(other.string());
					int i = 0;
					// prevent exception via unformatLinear
					if (sInput.find(L',') > 0) {
		...
		if (GETBIT(options, OdEd::kGptNoLimCheck) == false && database()->getLIMCHECK()) {
			OdGeExtents2d ext(database()->getLIMMIN(), database()->getLIMMAX());
			if (ext.contains((const OdGePoint2d&)res) == false) {
...
int ExDbCommandContext::getInt(const OdString& sPrompt, int options, int defVal, const OdString& sKeywords, OdEdIntegerTracker* pTracker)
{
	OdString prompt(sPrompt);
	if (prompt.isEmpty()) {
	...
	while (true) {
		OdString sInput = getString(prompt, options, OdString::kEmpty, sKeywords);
		if (sInput.isEmpty()) {
			return defVal;
		}

		try {
			return OdUnitsFormatterTool::toInt(sInput);
		}
		catch (const OdError& e) {
			if (e.code() != eInvalidInput) {
				throw;
			...
			putError(sKeywords.find(L' ') < 0 ? L"Requires integer value." : L"Requires integer value or optional keyword.");
		}
	}
}
...
int ExDbCommandContext::getKeyword(const OdString& sPrompt, const OdString& sKeywords, int defVal, int options, OdEdIntegerTracker* pTracker)
{
	OdString prompt(sPrompt);
	...
	if (bDef) {
		...
		if (prompt.find(L'<') < 0) {
			OdString defValPmt = ::findDefOptionPrompt(sPrompt, defVal);
			if (!defValPmt.isEmpty()) {
				prompt += L"<" + defValPmt + L">";
			}
		}
	}
	prompt += OD_T(":");
	...
	while (true) {
		...
		OdString sInput;
		try {
			OdEdTrackerProxy<OdEdIntegerTracker> trackerProxy;
			trackerProxy.tracker = pTracker;
			// ODE: overridden getString is a key method to support auto-urls by keywords in prompt
			sInput = getString(prompt, options, bDef ? kwIndex.at(defVal).keyword() : OdString::kEmpty, sKeywords, pTracker ? &trackerProxy : 0);
		}
		catch (OdEdKeyword& key) {
			return key.keywordIndex();
		}
...
double ExDbCommandContext::getReal(const OdString& sPrompt, int options, double defVal, const OdString& sKeywords, OdEdRealTracker* pTracker)
{
	OdString prompt(sPrompt);
	...
	OdEdTrackerProxy<OdEdRealTracker> trackerProxy;
	trackerProxy.tracker = pTracker;

	while (true) {
		OdString sInput = getString(prompt, options, OdString::kEmpty, sKeywords, pTracker ? &trackerProxy : 0);
		if (sInput.isEmpty())
			return defVal;
		try {
			...
			putError(sKeywords.find(L' ') < 0 ? L"Requires real value." : L"Requires real value or optional keyword.");
		}
	}
}
...
OdString ExDbCommandContext::getString(const OdString& sPrompt, int options, const OdString& pDefVal, const OdString& sKeywords, OdEdStringTracker* pTracker)
{
...
OdString ExDbCommandContext::getFilePath(const OdString& prompt, int options, const OdString& dialogCaption, const OdString& defExt, const OdString& fileName, const OdString& filter, const OdString& keywords, OdEdStringTracker* pTracker)
{
	OdString sInput;
...
OdCmColor ExDbCommandContext::getColor(const OdString& sPrompt, int options, const OdCmColor* pDefVal, const OdString& sKeywords, OdEdColorTracker* pTracker)
{
	// throw OdError(eNoDatabase) if it is not DWG
	database();
...
OdSharedPtr<OdCmColorBase> ExDbCommandContext::getCmColor(const OdString& sPrompt, int options, const OdCmColorBase* pDefVal, const OdString& sKeywords, OdEdColorTracker* pTracker)
{
	OdString prompt(sPrompt);
	...
	while (true) {
		OdString sInput = getString(prompt, options, pDefVal ? (const OdString&)baseFormatter().formatCmColor(*pDefVal) : OdString::kEmpty, sKeywords);
		try {
			...
			putError(sKeywords.find(L' ') < 0 ? L"Invalid color value.": L"Requires color value or optional keyword.");
		}
	}
}
