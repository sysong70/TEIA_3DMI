...
BEGIN_MESSAGE_MAP(CtlTreeEx, CBCGPTreeCtrlEx)
	ON_WM_LBUTTONDOWN()
END_MESSAGE_MAP()
...
bool CtlTreeEx::Initialize(CWnd* pParentWnd, UINT id, const RECT& rect)
{
...
bool CtlTreeEx::InitializeDesign(Json::Array& design)
{
	SetRedraw(FALSE);
	Initialized = false;
	{
...
void CtlTreeEx::GetAncestor(HTREEITEM pItem, std::vector<HTREEITEM>& ancestor)
{
...
CString CtlTreeEx::GetItemNamePath(HTREEITEM pItem)
{
...
void CtlTreeEx::OnLButtonDown(UINT nFlags, CPoint point)
{
	UINT flag = 0;
...
HTREEITEM CtlTreeEx::CreateItem(Json::Object& design, HTREEITEM pParent)
{
	if (design.GetBoolean("visible", true) == false) {
		return nullptr;
	}

	HTREEITEM pItem = InsertItem(Ast::GetTitle(design), pParent);
...
void CtlTreeEx::CreateItem(Json::Array& design, HTREEITEM pParent)
{
