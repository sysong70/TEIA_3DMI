...
#define DEFINE_MESSAGE(wm){ wm, L#wm }
...
IMPLEMENT_DYNCREATE(CTracerClientView, CView)

BEGIN_MESSAGE_MAP(CTracerClientView, CView)
END_MESSAGE_MAP()



CTracerClientView::CTracerClientView() noexcept
{
}



CTracerClientView::~CTracerClientView()
{
}



BOOL CTracerClientView::PreCreateWindow(CREATESTRUCT& cs)
{
	return CView::PreCreateWindow(cs);
}

#define TRACE theApp.Trace

BOOL CTracerClientView::PreTranslateMessage(MSG* pMsg)
{
	//if (pMsg->hwnd == GetSafeHwnd()) {
	//}
	const AFX_MAP_MESSAGE* map = allMessages;
	for (; map->string != NULL; map++) {
		if (map->message == pMsg->message) {
			TRACE(map->string);
			break;
		}
	}
...
void CTracerClientView::OnDraw(CDC* /*pDC*/)
{
	CTracerClientDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	if (!pDoc) {
		return;
	}
...
CTracerClientDoc* CTracerClientView::GetDocument() const
{
	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CTracerClientDoc)));
	return (CTracerClientDoc*)m_pDocument;
}
